...
// Statuses that require amount
const AMOUNT_STATUSES = ['PTP', 'Received', 'Partial Received'];
...
export default function NewVisit() {
  ...
  const handleLoanSubmit = (e: FormEvent) => {
    ...
    if (!/^\d{21}$/.test(val)) { setLoanError('Loan number must be exactly 21 numeric digits'); return; }
    setLoanError('');
    setLoanNumber(val);
    setStep('visit-form');
  };
  ...
  const openCamera = async () => {
    ...
    try {
      ...
      if (error.name === 'NotAllowedError') setCameraError('Camera permission denied. Please allow camera in browser settings.');
      else if (error.name === 'NotFoundError') setCameraError('No camera found on this device.');
      else setCameraError('Could not open camera: ' + error.message);
      setShowCamera(true);
    }
  };

  const closeCamera = () => { stopStream(); setShowCamera(false); setCameraError(''); setCameraReady(false); };
  ...
  const capturePhoto = () => {
    ...
    canvas.toBlob((blob) => {
      if (!blob) return;
      addPhotoFiles([new File([blob], `photo_${Date.now()}.jpg`, { type: 'image/jpeg' })]);
      closeCamera();
    }, 'image/jpeg', 0.9);
  };
  ...
  const addPhotoFiles = (newFiles: File[]) => {
    ...
    for (const file of newFiles) {
      if (!['image/jpeg', 'image/png'].includes(file.type)) { setPhotoError('Only JPEG and PNG files are allowed'); return; }
      if (photos.length + validFiles.length >= 5) { setPhotoError('Maximum 5 photos allowed'); return; }
      totalSize += file.size;
      if (totalSize > 10 * 1024 * 1024) { setPhotoError('Total photo size must not exceed 10 MB'); return; }
      validFiles.push(file);
    }
    setPhotos(prev => [...prev, ...validFiles]);
    Promise.all(validFiles.map(file => new Promise<string>(resolve => {
  ...
  const addPhotos = (files: FileList | null) => { if (files) addPhotoFiles(Array.from(files)); };
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setSubmitError('');
    if (geo.status !== 'granted' || !geo.latitude || !geo.longitude) { setSubmitError('Location access is required to submit visit.'); return; }
    if (photos.length === 0) { setSubmitError('At least 1 photo is required.'); return; }
    if (!personVisited.trim() || !status || !comments.trim()) { setSubmitError('All fields are required.'); return; }

    if (AMOUNT_STATUSES.includes(status)) {
      if (!ptpAmount || Number(ptpAmount) <= 0) { setSubmitError('Amount must be a positive number.'); return; }
    }
    if (status === 'PTP') {
    ...
    try {
      ...
      await api.submitVisit({
        ...
        address: geo.address,
        ...(AMOUNT_STATUSES.includes(status) && { ptp_amount: Number(ptpAmount) }),
        ...(status === 'PTP' && { ptp_date: ptpDate }),
      });
      setSubmitted(true);
          ...
          <form onSubmit={handleLoanSubmit} className="space-y-4">
            <div>
              ...
              <input id="loan-id" type="text" inputMode="numeric" maxLength={21}
                ...
                placeholder="Enter 21-digit loan number" value={loanInput}
  ...
  return (
    <div className="p-4 max-w-lg mx-auto pb-8">
      ...
      {showCamera && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, backgroundColor: '#000', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px' }}>
            ...
            <button onClick={closeCamera} style={{ color: '#fff', fontSize: '24px', background: 'none', border: 'none', cursor: 'pointer' }}>✕</button>
          </div>
          {cameraError ? (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
              <div style={{ background: '#7f1d1d', borderRadius: '12px', padding: '20px', textAlign: 'center', maxWidth: '300px' }}>
                ...
                <button onClick={closeCamera} style={{ background: '#fff', color: '#7f1d1d', border: 'none', borderRadius: '8px', padding: '8px 20px', fontWeight: 600, cursor: 'pointer' }}>Close</button>
              </div>
            </div>
          ) : (
            <>
              ...
              <div style={{ padding: '32px', display: 'flex', justifyContent: 'center', backgroundColor: '#111' }}>
                <button onClick={capturePhoto} style={{ width: '80px', height: '80px', borderRadius: '50%', backgroundColor: '#fff', border: '4px solid #9ca3af', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.5)' }}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => { setStep('loan-entry'); setLoanInput(''); }} className="text-xs text-gray-500 underline">Change</button>
      </div>
      ...
      <div className="card mb-5">
        ...
        <div className={`rounded-xl p-3 mb-4 text-sm flex items-start gap-2 ${
          ...
          {geo.status === 'requesting' && (
            <><svg className="animate-spin w-4 h-4 text-blue-600 mt-0.5 flex-shrink-0" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" /></svg>
            <span className="text-blue-700 font-medium">Fetching your location...</span></>
          )}
          {geo.status === 'granted' && (
            <><svg className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" /></svg>
            <div><p className="text-green-800 font-semibold">Location captured</p><p className="text-green-700 text-xs mt-0.5">{geo.address}</p><p className="text-green-600 text-xs font-mono">{geo.latitude?.toFixed(6)}, {geo.longitude?.toFixed(6)}</p></div></>
          )}
          {geo.status === 'denied' && (
            <><svg className="w-4 h-4 text-red-600 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
            <div>
              <p className="text-red-700 font-semibold">Location access blocked</p>
              <p className="text-red-600 text-xs mt-1">
                <strong>Chrome/Android:</strong> Address bar ke paas 🔒 → Site Settings → Location → Allow<br/>
                <strong>Safari/iPhone:</strong> Settings app → Safari → [is website] → Location → Allow<br/>
                Phir page refresh karo.
              </p>
              <button onClick={() => window.location.reload()} className="mt-2 text-xs bg-red-600 text-white px-3 py-1.5 rounded-lg font-semibold">Refresh Page</button>
            </div></>
          )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" readOnly value={loanNumber} className="input-field bg-surface-1 text-gray-500 cursor-not-allowed font-mono" />
          </div>

          <div>
            <label className="label" htmlFor="person-visited">Person Name Visited</label>
            <input id="person-visited" type="text" className="input-field" placeholder="Full name of person met"
          ...
          <div>
            ...
            <select id="status" className="input-field" value={status}
              onChange={e => { setStatus(e.target.value); if (!AMOUNT_STATUSES.includes(e.target.value)) { setPtpDate(''); setPtpAmount(''); } }} required>
              <option value="">Select status...</option>
              {VISIT_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>

          {/* Amount field — for PTP, Received, Partial Received */}
          {AMOUNT_STATUSES.includes(status) && (
            <div className={`rounded-xl p-4 space-y-4 ${
              status === 'PTP' ? 'bg-blue-50 border border-blue-200' :
              status === 'Received' ? 'bg-green-50 border border-green-200' :
              'bg-yellow-50 border border-yellow-200'
            }`}>
              <p className={`text-xs font-semibold uppercase tracking-wide ${
                status === 'PTP' ? 'text-blue-700' :
                status === 'Received' ? 'text-green-700' : 'text-yellow-700'
              }`}>
                {status === 'PTP' ? 'Promise to Pay Details' :
                 status === 'Received' ? 'Amount Received' : 'Partial Amount Details'}
              </p>

              {/* PTP Date — only for PTP */}
              {status === 'PTP' && (
                <div>
                  <label className="label" htmlFor="ptp-date">PTP Date <span className="text-red-500">*</span></label>
                  <input id="ptp-date" type="date" className="input-field"
                    value={ptpDate} min={new Date().toISOString().split('T')[0]}
                    onChange={e => setPtpDate(e.target.value)} required />
                </div>
              )}

              {/* Amount — for all three statuses */}
              <div>
                <label className="label" htmlFor="ptp-amount">
                  {status === 'PTP' ? 'Promised Amount (₹)' :
                   status === 'Received' ? 'Amount Received (₹)' : 'Partial Amount (₹)'}
                  <span className="text-red-500"> *</span>
                </label>
                <input id="ptp-amount" type="number" inputMode="numeric" className="input-field"
                  placeholder="Enter amount" value={ptpAmount} min={1}
          ...
          <div>
            ...
            <textarea id="comments" className="input-field resize-none" rows={3} placeholder="Add visit notes..."
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {photoPreviews.map((src, i) => (
                  <div key={i} className="relative">
                    <img src={src} alt={`Photo ${i + 1}`} className="w-20 h-20 object-cover rounded-xl border-2 border-surface-3" />
                    <button type="button" onClick={() => removePhoto(i)}
                      className="absolute -top-2 -right-2 w-5 h-5 bg-red-600 text-white rounded-full text-xs flex items-center justify-center">×</button>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* Past Visits */}
      <div>
        <h3 className="font-bold text-gray-900 mb-3">Past Visits <span className="ml-2 text-sm font-normal text-gray-500">for this loan</span></h3>
        {loadingPast && <div className="text-center py-6"><div className="animate-spin rounded-full h-6 w-6 border-2 border-brand-600 border-t-transparent mx-auto" /></div>}
        {!loadingPast && pastVisits.length === 0 && <div className="card text-center py-8 text-gray-400"><p>No previous visits for this loan</p></div>}
        {!loadingPast && pastVisits.map(v => (
          <div key={v.id} className="card mb-3">
            ...
            {/* Amount details for PTP / Received / Partial Received */}
            {AMOUNT_STATUSES.includes(v.status) && v.ptp_amount && (
              <div className={`rounded-lg px-3 py-2 mb-2 text-xs flex flex-wrap gap-3 ${
                v.status === 'PTP' ? 'bg-blue-50 border border-blue-200 text-blue-800' :
                v.status === 'Received' ? 'bg-green-50 border border-green-200 text-green-800' :
                ...
                <span>💰 {v.status === 'PTP' ? 'Promised' : v.status === 'Received' ? 'Received' : 'Partial'}: <strong>₹{v.ptp_amount?.toLocaleString('en-IN')}</strong></span>
              </div>
            )}

            <div className="flex items-start gap-1 text-xs text-gray-500 mb-3">
              <svg className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /></svg>
              <span>{v.address}</span>
            </div>
